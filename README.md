...
<a href="https://seedance-studio-five.vercel.app/">
  <img src="public/icons/icon-512.png" alt="Seedance Studio 图标，点击打开在线版" width="96" height="96">
</a>
...
- 若部署者启用可选的 Cloudflare 大图通道，大图生成的 Key 和图片会临时经过该 Worker；页面会显示代理地址。小图和任务查询仍经过 Next.js。
...
3. 可添加最多 10 张 PNG、JPEG 或 WebP 参考图。默认单张及总大小不超过 3 MB；站点启用 Cloudflare 大图通道后，单张最多 15 MB、总计最多 45 MB，不压缩原图。以页面提示和模型实际支持为准。
...
4K 输出可能使用 HEVC / 10-bit 编码，部分浏览器无法直接预览，但仍可下载。默认图片单张及总量均不超过 3 MB，序列化请求不超过 4,000,000 字节；可选大图通道为单张 15 MB、总计 45 MB（文件大小按 MiB 计算）。最终提示词上限为 4,000 字符。切换模型时，应用会调整不适用的参数并提示，但不会自动删除或调换参考图。
...
在自己的 Cloudflare 账号部署仓库内的 Worker，再配置 `NEXT_PUBLIC_SEEDANCE_WORKER_ORIGIN` 并重新构建 Vercel。用户仍只需填写自己的火山 Key；无需 Blob/R2 存储。所有访问者共享部署者的 Worker 配额。

参见 [部署与验证说明](docs/cloudflare-image-proxy.md)。**先在 Preview 验证免费套餐 CPU、15 MB 图片及火山兼容性，再启用 Production**；本地测试不代表线上免费额度已经验证。未配置时仍使用原小图通道。

**本站状态（2026-09-23）：**大图 Worker 已部署，跨域预检已通过；公开网站尚未配置该通道，因此目前仍按页面显示的默认小图上限使用。15 MB 真实生成尚未验证。
...
启用大图通道时，超出原请求上限的图片改走 `浏览器 → Cloudflare Worker（流式校验）→ 火山方舟`，不会经过 Vercel 的大请求入口；任务查询仍走上图链路。
